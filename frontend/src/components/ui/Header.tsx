  ...
  <header className="px-10 lg:px-16 py-6 flex items-center">
    ...
    <nav className="ml-auto flex gap-4 sm:gap-6">
      {["Contact"].map((item, index) => (
        <a key={index} className="text-lg font-medium hover:underline underline-offset-4 font-primaryRegular" href="https://ronitkhajuria.vercel.app" rel="ugc" target="_blank">
          {item}
        </a>
      ))}
    </nav>
